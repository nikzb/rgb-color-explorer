...
class ButtonGroupMenu extends Component {
  render() {
    ...
    const buttonList = buttons.map((button) => {
      return <button key={button.label} className={`button ${button.classes}`} onClick={button.onClick} onMouseDown={button.onMouseDown} onMouseUp={button.onMouseUp}>{button.label}</button>
    });
